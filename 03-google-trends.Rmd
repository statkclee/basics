...
zeitgeist %>% 
  write_rds("data/zeitgeist_20210530.rds")
```

```{r gtrends-EDA}
zeitgeist <- read_rds("data/zeitgeist_20210530.rds")
...
## 연관키워드 시대정신  {#keywords-by-keyword}
...
```{r gtrends-EDA-linked-keywords, eval = FALSE}
library(ggraph)
library(tidygraph)
...
keyword_tidy_nw <- tbl_graph(
  nodes = keyword_node_tbl, edges = keyword_edge_tbl, directed = TRUE
)
